...
export default function BabEnamMenu({ navigation }: RootStackScreenProps<"babEnamMenu">) {
    ...
    const color = "red"
    const centerNode = { x: centerX, y: centerY - outerRadius + 30, image: MindMapPengumpulan, color: color };


    const topExtraNodes = [
        { x: centerNode.x - innerCircleRadius * Math.cos(Math.PI / 3) * 1.5, y: centerNode.y - innerCircleRadius * Math.sin(Math.PI / 3) * 1, color: color, title: "Mengumpulkan dan Mengolah Data" },
        { x: centerNode.x, y: centerNode.y - innerCircleRadius * 1.5, color, title: "Menyajikan Data" },
        { x: centerNode.x + innerCircleRadius * Math.cos(Math.PI / 3) * 1.5, y: centerNode.y - innerCircleRadius * Math.sin(Math.PI / 3) * 1, color: color, title: "Membaca Data" },
    ];
    ...
    const handleImagePress = (index: number, pos: string, title?: string) => {
        ...
        if (pos === "Top") {
            ...
            navigation.navigate("babEnamMenuSub", { title: title! })
