...
export default function BabSatuMenu({ navigation }: RootStackScreenProps<"babSatuMenu">) {
    ...
    const centerNode = { x: centerX, y: centerY - outerRadius + 30, image: MindMapKpk, color: 'orange' };


    const topExtraNodes = [
        { x: centerNode.x - innerCircleRadius * Math.cos(Math.PI / 3) * 1.5, y: centerNode.y - innerCircleRadius * Math.sin(Math.PI / 3) * 1, color: "orange", title: "KPK", colors: ['#e52d27', '#b31217'] },
        { x: centerNode.x + innerCircleRadius * Math.cos(Math.PI / 3) * 1.5, y: centerNode.y - innerCircleRadius * Math.sin(Math.PI / 3) * 1, color: "orange", title: "FPB", colors: ["#061700", "#52c234"] },
    ];
    ...
    const handleImagePress = (index: number, pos: string) => {
        ...
        if (pos === "ExtraBot") {
            ...
            navigation.navigate("babSatuMenuSub", {
                title: title, another: another
            })
        }
        else if (pos === "Top") {
            if (index === 2) {
                ...
                <View style={{}}>
                    ...
                    {topExtraNodes.map((node, index) => (
                        <TouchableWithoutFeedback key={index + 9}
                            onPress={() => handleImagePress(index, "ExtraBot")}>
                            <View style={{ position: 'absolute', top: node.y - 50, left: node.x - 50, width: 100, height: 100, borderColor: node.color, borderWidth: 5, backgroundColor: 'white', borderRadius: 50, justifyContent: 'center', alignItems: 'center' }} >
                                <Text style={{ color: 'black' }}>{node.title}</Text>
                            </View>
