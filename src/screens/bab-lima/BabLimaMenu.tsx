...
import { BABV } from '../../constants/BabConstant';

export default function BabLimaMenu({ navigation }: RootStackScreenProps<"babLimaMenu">) {
    ...
    const color = "green"
    const centerNode = { x: centerX, y: centerY - outerRadius + 30, image: MindMapBangunRuang, color: color };


    const topExtraNodes = [
        { x: centerNode.x - innerCircleRadius * Math.cos(Math.PI / 3) * 1.5, y: centerNode.y - innerCircleRadius * Math.sin(Math.PI / 3) * 1, color: color, title: BABV.SBI },
        { x: centerNode.x, y: centerNode.y - innerCircleRadius * 1.5, color, title: BABV.SBII },
        { x: centerNode.x + innerCircleRadius * Math.cos(Math.PI / 3) * 1.5, y: centerNode.y - innerCircleRadius * Math.sin(Math.PI / 3) * 1, color: color, title: BABV.SBIII },
    ];
    ...
    const bottomNodes = [
        ...
        { x: centerX, y: centerY + outerRadius * 1.3 / 2, image: MindMapBangunDatar, color: 'yellow' },
    ...
    const handleImagePress = (index: number, pos: string, title?: string) => {
        ...
        if (pos === "Top") {
            ...
            navigation.navigate("babLimaMenuSub", { title: title! })
        }
    };
